...
public class HomeAdapter extends BaseQuickAdapter<ListCouseInfo.ResultBean,BaseViewHolder> {

    Drawable right_vip;
    Drawable right_free;

    public HomeAdapter(int layoutResId, List<ListCouseInfo.ResultBean> data) {
    ...
    @Override
    protected void convert(BaseViewHolder holder, ListCouseInfo.ResultBean info) {
        if(info==null)
            return;

        SimpleDraweeView ivIcon = holder.getView(R.id.dv_img);
//        ivIcon.setImageURI(info.);
        holder.setText(R.id.tv_date, info.task_minute+"");
        holder.setText(R.id.tv_city, info.target+"");

        holder.setText(R.id.tv_label, info.level+"");
